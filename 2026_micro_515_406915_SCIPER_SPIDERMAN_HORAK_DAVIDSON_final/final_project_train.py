...
def _resolve_phase2_seed_path(seed_path: str) -> str:
    """Resolve a phase-2 seed to a training checkpoint path.

    Evaluation videos store a copied genotype plus metadata. We accept those
    artifacts only when they can be traced back to a real training checkpoint.
    """
    resolved_path = os.path.abspath(seed_path)
    ...
    if basename in {"video_full_genotype.npy", "video_controller_genotype.npy"}:
        ...
        source_path = info.get("source_genotype_path")
        if not source_path:
            raise ValueError(
                f"Warmstart artifact '{seed_path}' does not record a source_genotype_path"
            )

        resolved_path = os.path.abspath(source_path)
        if not os.path.isfile(resolved_path):
            raise FileNotFoundError(
                f"Resolved training checkpoint not found: {resolved_path}"
            )

    parts = os.path.normpath(resolved_path).split(os.sep)
    if os.path.basename(resolved_path) != "x_best.npy" or "results" not in parts:
        raise ValueError(
            "Phase-2 warmstarts must resolve to a training checkpoint x_best.npy under results/. "
            f"Got: {resolved_path}"
        )
...
class FinalWorld(World):
    ...
    def __init__(self, fixed_weights=None):
        ...
        self.controller = CPGControllerWithPID(self.controller)
...
def run_cmaes_refinement(
    ...
    if seed_genotype is not None:
        seed_score_vec = world.evaluate_individual(
            seed_genotype, n_repeats=n_repeats, n_steps=n_steps
        )
        seed_score = float((seed_score_vec * np.array([1.0, 1.0, 2.0])).sum())
        print(
            f"Warm-start baseline  score={seed_score:.2f}  "
            f"objectives={seed_score_vec.tolist()}"
        )
    ...
    for gen in range(num_generations):
        ...
        if gen == 0:
            ...
            if warmstart_vector is not None:
                pop[0] = np.asarray(warmstart_vector, dtype=float).copy()
...
if __name__ == "__main__":
    ...
    if args.phase2:
        if args.seed_path is not None:
            seed_path = _resolve_phase2_seed_path(args.seed_path)
