"""Interactive Central Pattern Generator (CPG) based control with P-controller for the robot actuators.

This script implements a simplified CPG approach to generate rhythmic locomotion patterns.
It allows manual tuning of CPG parameters:
- Global frequency (speed)
- Knee/Hip frequency ratio
- Phase shifts (knee relative to hip, inter-leg)
- Hip amplitude and offset
- Knee lift amplitude and swing duration ratio
- P-gains for each actuator

The gait is designed as a "4-temps" pattern where one leg is in the air at a time,
with configurable swing phase duration.

Usage:
  py cpg_controller.py --env HillEnv-v0
"""
...
def main() -> None:
    ...
    knee_indices = [i for i, name in enumerate(labels) if "knee" in name.lower() or "ankle" in name.lower()]
    if not hip_indices and not knee_indices:
        hip_indices = list(range(0, n_actions, 2))
        knee_indices = list(range(1, n_actions, 2))

    root = tk.Tk()
    root.title(f"CPG Controller - {args.env}")
    ...
    osc_params_config = [
        ("speed", "Global Speed (mult of 1 rad/s)", 0.0, 20.0, 8.0),
        ...
        (
            "swing_duration_ratio",
            "Swing Duration Ratio (0-1)",
            0.1,
            0.9,
            0.4,
        ),  # New slider
        ("h_amp", "Hip Amplitude", 0.0, 1.0, 0.4),
        ("h_off", "Hip Offset", -1.0, 1.0, 0.0),
        (
            "knee_lift_amplitude",  # Renamed
            "Knee Lift Amplitude (0-1)",
            0.0,
            1.0,
            1.0,
    ...
    # Add slider for rear leg amplitude scaling
    osc_params_config.append(
        ("rear_leg_scale", "Rear Leg Amplitude Scale", 0.5, 2.0, 1.2)
    )

    # Ensure hip_indices and knee_indices are correctly identified for default K values
    # This part is already handled in the loop below, but good to keep in mind.
    ...
    for idx, name in enumerate(labels):
        ...
        # Set default K based on whether it's a hip or knee
        if idx in hip_indices:
            k_slider.set(0.1)  # Increased default for hips
        elif idx in knee_indices:
            k_slider.set(2.5)  # Increased default for knees
    ...
    fig_traj = Figure(figsize=(5, 5), dpi=80)
    ax_traj = fig_traj.add_subplot(111)
    (line_traj,) = ax_traj.plot([], [], "b-", linewidth=1.5)
    ax_traj.set_title("Torso Trajectory (X, Y)")
    ax_traj.set_xlabel("X (m)")
    ax_traj.set_ylabel("Y (m)")
    ax_traj.grid(True)

    traj_window = tk.Toplevel(root)
    traj_window.title("Robot Trajectory")
    canvas_traj = FigureCanvasTkAgg(fig_traj, master=traj_window)
    ...
    for i in range(n_actions):
        ...
        if model.jnt_limited[joint_id]:
            ...
            min_y_initial = min(min_y_initial, low - 0.1)
            max_y_initial = max(max_y_initial, high + 0.1)
        ax_left.set_ylim(min_y_initial, max_y_initial)
    ...
    progress_label = ttk.Label(
        controls, text="Distance: 0.00m", font=("Arial", 11, "bold")
    )
    progress_label.pack(side="top", pady=5)

    def _reset_env() -> None:
        nonlocal sim_time, x_history, y_history
        sim_time = 0.0
        x_history.clear()
        y_history.clear()
        env.reset(seed=args.seed)
        ...
        for idx, ks in enumerate(k_sliders):
            if idx in hip_indices:
                ...
                ks.set(2.5)
            else: # Fallback for any other actuators
                ks.set(1.0)
    ...
    def _loop() -> None:
        ...
        swing_duration_ratio = oscillator_sliders[
            "swing_duration_ratio"
        ].get()  # New parameter
        h_amp = oscillator_sliders["h_amp"].get()
        h_off = oscillator_sliders["h_off"].get()
        knee_lift_amplitude = oscillator_sliders["knee_lift_amplitude"].get()  # Renamed
        rear_leg_scale = oscillator_sliders["rear_leg_scale"].get() # New parameter
        ...
        for idx in range(n_actions):
            ...
            # Symmetry for Ant-v4: Right (0, 3) vs Left (1, 2)
            side_sign = 1.0 if leg_idx in [0, 3] else -1.0
            back_sign = -1.0 if leg_idx >= 2 else 1.0

            if is_hip:
                # Propulsion logic:
                # Swing: move smoothly from backward (-h_amp) to forward (+h_amp)
                # Stance: push linearly from forward (+h_amp) to backward (-h_amp)
                if phi < swing_duration_ratio:
                    alpha = phi / swing_duration_ratio
                    # Sine swing for smoothness
                    cycle_val = -math.cos(math.pi * alpha) 
                else:
                    beta = (phi - swing_duration_ratio) / (1.0 - swing_duration_ratio)
                    # Linear push for constant propulsion
                    cycle_val = 1.0 - 2.0 * beta

                # Apply rear leg scale to hip amplitude
                current_h_amp = h_amp * (rear_leg_scale if leg_idx >= 2 else 1.0)

                # Apply side and back signs to ensure all legs push backward in world coordinates
                target_angle = h_off + (side_sign * back_sign * current_h_amp * cycle_val)

            elif is_knee:
                ground_pos = side_sign * KNEE_GROUND_POS
                air_pos = side_sign * KNEE_AIR_POS

                if phi < swing_duration_ratio:  # Active window (swing phase)
                    alpha = phi / swing_duration_ratio  # map to [0, 1]
                    # Knee lift: half-sine wave
                    lift_factor = math.sin(math.pi * alpha)

                    # Apply rear leg scale to knee lift amplitude
                    current_knee_lift_amplitude = knee_lift_amplitude * (rear_leg_scale if leg_idx >= 2 else 1.0)

                    lift_movement = (air_pos - ground_pos) * lift_factor
                    target_angle = ground_pos + current_knee_lift_amplitude * lift_movement
                else:  # Stance window: Stay at ground contact
        ...
        # Update Trajectory Plot and Progress
        try:
            # qpos[0] and qpos[1] are world coordinates in MuJoCo Ant
            torso_x = env.unwrapped.data.qpos[0]
            torso_y = env.unwrapped.data.qpos[1]
            x_history.append(torso_x)
            y_history.append(torso_y)
            ...
            # Distance from origin as a measure of progress
            dist = math.sqrt(torso_x**2 + torso_y**2)
            progress_label.config(text=f"Distance: {dist:.2f}m")
        except Exception:
            pass
        ...
        for i in range(n_actions):
            ...
            # Dynamic Y-axis adjustment for position plots
            ax_left = axes[i // 2, (i % 2) * 2]
            all_pos_data = list(target_angle_history[i]) + list(
                measured_angle_history[i]
            )
            if all_pos_data:
                current_min, current_max = ax_left.get_ylim()
                data_min = min(all_pos_data)
                data_max = max(all_pos_data)

                new_min = min(current_min, data_min - 0.1)
                new_max = max(current_max, data_max + 0.1)
                ax_left.set_ylim(new_min, new_max)
