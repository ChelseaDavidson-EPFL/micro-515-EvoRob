...
def record_robot(
    ...
    try:
        ...
        # Fix for imageio ffmpeg plugin calling .tostring() on NumPy 2.0+
        class FfmpegFrame(np.ndarray):
            def tostring(self):
                return self.tobytes()
        ...
        if len(frames) == 0:
            ...
            for frm in frames:
                ...
                writer.append_data(arr.view(FfmpegFrame))
            writer.close()
...
def main():
    ...
    parser.add_argument(
        "--directional",
        action="store_true",
        help="Interpret checkpoints as direction-conditioned models",
    )
    parser.add_argument(
        "--target_direction",
        type=float,
        ...
        default=(1.0, 0.0),
        metavar=("DX", "DY"),
        help="Direction vector used when replaying direction-conditioned checkpoints",
    )
    parser.add_argument(
        "--leg_layout",
        type=str,
